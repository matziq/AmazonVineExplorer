...
class AVE_EVENTHANDLER {
    ...
    */
     ...
     * @param {string} eventName That's the name of the event you want to listen for
     * @param {function} cb That's the function that gets called in case of this event
     */
    on(eventName, cb) {
...
SETTINGS_USERCONFIG_DEFINES.push({key: 'EnableDesktopNotification', type: 'bool', name: 'Enable Desktop Notifications', description: 'Enable Desktop Notifications if new Products are detected'});
...
SETTINGS_USERCONFIG_DEFINES.push({key: 'DesktopNotificationDelay', type: 'number', min: 1, max: 900, name: 'Desktop Notification Delay(Seconds)', description: 'Minimal Time between Desktop Notifications. except Notifications with matched keywords'});
...
class SETTINGS_DEFAULT {
    ...
    DesktopNotificationDelay = 60;
    DesktopNotificationKeywords = [];
...
async function fastStyleChanges() {
    ...
    if (SITE_IS_VINE) {
        ...
        if (SETTINGS.EnableTopLogoChange) {
            waitForHtmlElmement('#vvp-logo-link > img', (elem) => {
                elem.src = 'https://raw.githubusercontent.com/matziq/AmazonVineExplorer/i18n/english-ui/vine_logo_notification_image.png';
